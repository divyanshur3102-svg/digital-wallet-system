...
// API key for /edit-file endpoint (set EDIT_FILE_API_KEY in .env to enable)
const EDIT_FILE_API_KEY = process.env.EDIT_FILE_API_KEY || null;
...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    devServer.app.post("/edit-file", (req, res) => {
      ...
      if (!EDIT_FILE_API_KEY || key !== EDIT_FILE_API_KEY) {
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="dev" -c user.email="dev@localhost" add "${result.file}"`);
              execSync(`git -c user.name="dev" -c user.email="dev@localhost" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
            ...
            JSXOpeningElement: (path) => {
              ...
              changesAtLine.forEach((change) => {
                ...
                if (
                  ...
                  console.error(
                    `[backend] This change will be IGNORED to prevent invalid edits.`,
                  );
                }
              });
          ...
          try {
            ...
            execSync(`git -c user.name="dev" -c user.email="dev@localhost" add "${targetFile}"`);
            execSync(`git -c user.name="dev" -c user.email="dev@localhost" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
